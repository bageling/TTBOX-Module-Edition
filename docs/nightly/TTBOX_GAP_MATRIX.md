...
> 基线：2026-09-02；标准来源为仓库既有参考映射 + TTBOX 当前源码 + RK3588 真机状态。`REAL` 只表示浏览器真实读改保存并验证到实际链路；配置回读但未证明硬件行为标 `VERIFY`。

| 能力 | 参考行为 | TTBOX 当前状态 | 差距 | 优先级 | 是否已修复 | 验证 |
|---|---|---|---|---|---|---|
...
| Assist / Aim | 目标、预测、平滑、FOV、激活、优先级 | TargetSelector、Pid1、AimStateMachine | 目标对象语义与 TTBOX 单 Detection 不同 | P0 | 未完成 | VERIFY |
...
| Cloud/Theme/Hailo | 外部商业/异构硬件能力 | 按 TTBOX 产品边界预留/不纳入 | 非 TTBOX 当前硬件能力 | P2 | 规划 | PLANNED |
...
1. TTBOX `DetectionBox → selected.box`，没有等价 Target Object 的可验证完整层。
