...
浏览器
  ↓ HTTP
TTBOX Web 插件 (plugins/web, Flask, 0.0.0.0:8000)
...
| ttbox-web.service | enabled+active | 8000 | Web 插件（待切换） |
...
| preview | process | ✅ | bin/ttbox-preview → ttbox-preview.py | 低帧 JPEG/MJPEG 预览 8001 |
...
1. ✅ ttbox-web.service 切换到插件 Web（/opt/ttbox/plugins/web/bin/ttbox-web，0.0.0.0:8000），旧 Web（/opt/ttbox/web 8000）已停用
...
4. ✅ 新增 ttbox-preview.service（8001）并启用；web/preview 插件在 /api/plugins 中实时显示 running/healthy（HTTP 探活）
