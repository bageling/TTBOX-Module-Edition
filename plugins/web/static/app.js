...
function renderSystemStats(payload) {
  ...
  state.webPort = Number(payload.web_port) || state.webPort || 8000;
...
function webUrl(host, port = state.webPort || 8000) {
...
function syncWebPortInputs(payload) {
  ...
  const port = Number(payload && payload.web_port) || state.webPort || 8000;
  state.webPort = port;
  if (input && document.activeElement !== input) {
...
function networkAccessDraft() {
  ...
  return {
    ...
    portChanged: port !== Number(state.webPort || 8000),
  };
}
...
function bindEvents() {
  ...
  on("saveDisplayHardwareButton", "click", () => runUiAction(async () => {
    ...
    // 保存并应用 = 显式应用（总是注入 EDID + 触发 HPD 重新枚举），
    // 不依赖配置是否变化——用户点击按钮就是要应用当前配置
    const confirmed = window.confirm("保存并应用会写入 HDMI RX EDID 并触发重新检测。Windows 端可能需要重新枚举显示器，确认继续？");
    if (!confirmed) {
      return;
    }
    ...
    const result = await api("/api/hardware/display", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ config, apply: true, patch_boot_image: true, reboot_after_apply: false }),
    });
    populateDisplayHardware({
      available: true,
