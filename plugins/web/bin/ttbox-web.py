...
LISTEN_PORT = int(os.environ.get('TTBOX_WEB_PORT', '8000'))
...
def _license_payload() -> dict:
    ...
    license_data = {
        ...
        'license_id': 'lic_ttbox_local',
        'license_version': '2',
        'max_version': '9999.99.99',
        'message': '已激活',
        'online_checked_at': '',
        'online_grace_remaining_seconds': 0,
        'online_grant_valid_until': '',
        'status': 'activated',
        'valid': True,
    }
    ui = {
...
def activate_license():
    ...
    if not license_key:
        return jsonify({'ok': False, 'error': 'license_key is required'})
    # 真实激活：TTBOX 本地授权文件（无外部服务器）
    try:
        lic = _license_payload()
        if license_key == 'TTBOX-LOCAL':
...
def update_versions():
    ...
    return jsonify({'ok': True, 'data': {
        'components': {
            'core': {'current_version': ver, 'latest_version': ver, 'update_available': False},
            'usb_proxy': {'current_version': ver, 'latest_version': ver, 'update_available': False},
        },
        'latest_version': ver,
        'package': None,
...
def cleanup_stuck_update():
    ...
    idle_status = {
        ...
        'started_at': now, 'status': 'idle', 'type': '', 'unit': '',
        'updated_at': now, 'version': '',
    }
    return jsonify({'ok': True, 'data': {
        'backup': '', 'cleaned': True, 'deleted': [],
        'previous_status': dict(idle_status),
