...
bool DecodeNMS::process(const RknnModelInfo& info,
    ...
    // 多输出分发（顺序即优先级，与 ModelAdapter::infer_decode_type 对齐）：
    // 1) 3 的倍数 reg/cls/aux 三元组（大腕256：9 输出 = 3 尺度 × [reg,cls,aux]）
    // 2) 偶数成对 reg/cls（sjz-XCSH：6 输出，reg≥32 且 %4==0）
    // 3) 偶数成对 box/cls 通用 DFL（黄瓦：box [1,1,4,M]）
    if (info.n_outputs >= 6 && info.n_outputs % 3 == 0) {
        bool triple_reg_cls_aux = true;
        for (uint32_t i = 0; i + 2 < info.n_outputs; i += 3) {
            const auto& ro = info.outputs[i];
            const auto& co = info.outputs[i + 1];
            const auto& ao = info.outputs[i + 2];
            if (ro.dims.size() != 4 || co.dims.size() != 4 || ao.dims.size() != 4 ||
                ro.dims[1] < 32 || ro.dims[1] % 16 != 0 ||
                ao.dims[1] != 1 || co.dims[1] == 0) {
                triple_reg_cls_aux = false;
                break;
            }
        }
        if (triple_reg_cls_aux) return process_dfl_dist(info, out_bufs, detections, error);
    }
