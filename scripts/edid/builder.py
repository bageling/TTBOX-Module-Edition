...
def _pack_dtd(t: DisplayTiming) -> bytes:
    ...
    buf[12] = 0xA6  # h_image_size lo（RK3588 实测基线：16:9 2560x1440 → 698x392mm 分两字节编码）
    buf[13] = 0x7E
...
def _monitor_name_dtd(name: str) -> bytes:
    """显示器名称 DTD (0xFC)，对齐 RK3588 实测：名称 + \n + 2 空格填满 13 字节。"""
    buf = bytearray(18)
    buf[3] = 0xFC
...
def _serial_dtd(serial_text: str) -> bytes:
    """序列号描述符 DTD (0xFF)，对齐 RK3588 实测：11 字符 + \n + 空格 = 13 字节。"""
    buf = bytearray(18)
    buf[3] = 0xFF
    text = (serial_text[:11] + '\n ').encode('ascii', 'replace')
    buf[5:18] = text[:13]
...
def _range_limits_dtd_baseline(min_v: int = 239, max_v: int = 241,
                               min_h: int = 254, max_h: int = 255,
                               max_clock_mhz: int = 600) -> bytes:
    """频率范围 DTD (0xFD)，逐字节对齐 RK3588 实测 1440p144 成功版：
    ...
    buf[10] = 0x0A   # 扩展标志（实测基线 = 0x0A）
    return bytes(buf)


def _range_limits_dtd(max_clock_mhz: int) -> bytes:
    """根据最高广播时序生成 Range Limits，匹配 TTBOX 高刷能力声明。"""
    ...
    # RK3588 高刷 EDID 使用 254-255kHz 的安全范围，避免源端把
    ...
    return _range_limits_dtd_baseline(min_v, max_v, min_h, max_h, max_clock_mhz)


def ttbox_established() -> bytes:
    """Established timings 字节（25-34，共 10 字节），逐字节对齐 RK3588 实测成功版。"""
    return bytes.fromhex("cf74a3574cb02309484c")


def _hdmi_vsdb(max_tmds_mhz: int = 600) -> bytes:
    """HDMI Vendor Specific Data Block（CTA-861 扩展块内），字节格式对齐 RK3588 实测。
    根因修复 1：此前 EDID 无 HDMI VSDB → PC 显卡视为 DVI 设备 → 2K 被拒。
    根因修复 2：注册 ID 必须按 LSB-first 写（RK3588 实测 03 0c 00 = IEEE 0x000C03），
    ...
    布局（对齐 RK3588 实测 67 03 0c 00 10 00 00）：
      ...
      byte7 = 0x20 | max_tmds 位... 实测基线：byte7 高位为版本/能力位
    RK3588 1440p144 实测: 67 03 0c 00 10 00 00 76 67 d8 5d c4 01 76 80
      即 HDMI1.4 VSDB(8B) + HF-VSDB(7B): 76 67 d8 5d c4 01 76 80
    """
    # 逐字节对齐 RK3588 成功实测布局（1440p144 协商成功版）：
    ...
    # RK3588 实机成功样本使用 0x77，表示 SCDC/高带宽能力；0x76 会让部分源端退回 DVI。
...
def _build_cta_extension(dtds, native_index=0) -> bytes:
    """CTA-861 Extension Block (128B)，布局对齐 RK3588 实测成功版：
    - 数据块 = HDMI1.4 VSDB(8B) + HF-VSDB(8B)，dtd_start=20
    - 扩展块 DTD 区全空（时序只在基础块 DTD1 —— 实测基线如此，
      非标准模式混入扩展块反而让 PC 拒绝）
    """
    ext = bytearray(128)
    ext[0] = 0x02       # CTA-861
    ...
    # dtd_start = 20（实测基线同款：数据块后无 VIC 块、DTD 区留空）
    dtd_start = 4 + len(vsdb)
    ext[2] = dtd_start & 0xFF
    ext[3] = (dtd_start >> 8) & 0xFF
    # DTD 区留 0（实测基线；DTD0=0 表示"无额外 DTD"合法）
...
class EdidBuilder:
    ...
    def build(self) -> bytes:
        ...
        bb[16], bb[17] = 0x01, 0x22  # 制造周=1/年=2022（实测基线）
        bb[18], bb[19] = 1, 4        # EDID 1.4（HF-VSDB/HDMI2.0 规范要求 1.4，
                                     # 1.3 下 PC 忽略 HF-VSDB → 586MHz 被拒）
        bb[20] = 0xA2                # 数字输入: bit7=1, DVI bit3=1, bit5=1（实测基线）
        bb[21], bb[22] = 0, 0        # 屏幕尺寸: 0=投影/不确定（实测基线）
        bb[23] = 0x78                # Gamma 2.2
        # 对齐实测基线精确布局：byte24=feature(0x0A)，byte25-34=established timings
        bb[24] = 0x0A
        bb[25:35] = ttbox_established()  # CF 74 a3 57 4c b0 23 09 48 4c
        bb[35] = 0x00
        # standard timings (38-53)：全 0x01（实测基线：不广播标准时序兜底模式，
        ...
        if not self.native_only and len(dtds) < 3:
            ...
            for tok in ('1080p60compat', '1440p60', '2160p60'):
                try:
                    ...
                    pass
        bb[54:72] = _pack_dtd(dtds[0])                       # DTD1 首选（实测基线同布局）
        # 描述符顺序对齐实测基线：DTD2=名称、DTD3=序列号、DTD4=Range Limits
        bb[72:90] = _monitor_name_dtd(self.name)
        # 序列号文本格式对齐实测基线：vendor + serial hex 大写（ZWXDF6D7185）
